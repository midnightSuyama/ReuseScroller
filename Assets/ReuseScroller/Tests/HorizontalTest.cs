...
namespace ReuseScroller
{
	...
	public class HorizontalTest
	{
		...
		[SetUp]
		public void Init() {
			scrollView = TestHelper.CreateScrollView(Direction.Horizontal);
		...
		public void ContentAnchor() {
			...
			Assert.That(rt.anchorMax, Is.EqualTo(Vector2.up));
		}
		...
		public void CellOffset() {
			...
			Assert.That(rt.offsetMax, Is.EqualTo(new Vector2(101.0f, -3.0f)));
			rt = cells.First.Next.Value.GetComponent<RectTransform>();
			Assert.That(rt.offsetMin, Is.EqualTo(new Vector2(121.0f, 4.0f)));
			Assert.That(rt.offsetMax, Is.EqualTo(new Vector2(321.0f, -3.0f)));
		}
		...
		public IEnumerator ReuseCells() {
			...
			for (int i=0; i<=20; i++) {
				scrollRect.horizontalNormalizedPosition = (i<=10 ? i : 20-i) * 0.1f;
