...
namespace ReuseScroller
{
	...
	public class VerticalTest
	{
		...
		[SetUp]
		public void Init() {
			scrollView = TestHelper.CreateScrollView(Direction.Vertical);
		...
		public void ContentAnchor() {
			...
			Assert.That(rt.anchorMin, Is.EqualTo(Vector2.up));
		...
		public void CellOffset() {
			var cells = TestHelper.GetCells(scrollView.GetComponent<TestController>());
			var rt = cells.First.Value.GetComponent<RectTransform>();
			Assert.That(rt.offsetMin, Is.EqualTo(new Vector2(1.0f, -103.0f)));
			Assert.That(rt.offsetMax, Is.EqualTo(new Vector2(-2.0f, -3.0f)));
			rt = cells.First.Next.Value.GetComponent<RectTransform>();
			Assert.That(rt.offsetMin, Is.EqualTo(new Vector2(1.0f, -323.0f)));
			Assert.That(rt.offsetMax, Is.EqualTo(new Vector2(-2.0f, -123.0f)));
		}
		...
		public IEnumerator ReuseCells() {
			...
			for (int i=0; i<=20; i++) {
				scrollRect.verticalNormalizedPosition = (i<=10 ? 10-i : i-10) * 0.1f;
